  ...
  async getUserSpendings(userId: number, year: number, month: number) {
    ...
    const budgetSpendings = budgets.map((budget: BudgetWithCategory) => {
      ...
      const totalSpent = categoryTransactions.reduce(
        (sum: number, t: TransactionWithCategory) => {
          const amount =
    ...
    const budgetCategoryIds = new Set(
      budgets.map((b: BudgetWithCategory) => b.category_id),
    );
    const unmatchedTransactions = transactions
      .filter(
        (t: TransactionWithCategory) => !budgetCategoryIds.has(t.category_id),
      )
